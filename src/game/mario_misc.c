...
struct MarioBodyState gBodyStates[MAX_NUM_PLAYERS]; // 2nd is never accessed in practice, most likely Luigi related
...
Gfx *geo_switch_mario_stand_run(s32 callContext, struct GraphNode *node, UNUSED Mat4 *mtx) {
    ...
    struct MarioBodyState *bodyState = &gBodyStates[gCurGraphNodeObjectNode->oUnk94];
...
Gfx *geo_mario_tilt_torso(s32 callContext, struct GraphNode *node, UNUSED Mat4 *mtx) {
    ...
    if (callContext == GEO_CONTEXT_RENDER) {
        ...
        if (action != ACT_BUTT_SLIDE && action != ACT_HOLD_BUTT_SLIDE && action != ACT_WALKING) {
            bzero(bodyState->torsoAngle, sizeof(Vec3s));
        }
...
Gfx *geo_mario_hand_foot_scaler(s32 callContext, struct GraphNode *node, UNUSED Mat4 *mtx) {
    ...
    static s16 sMarioAttackAnimCounter[MAXCONTROLLERS];
    struct GraphNodeGenerated *asGenerated = (struct GraphNodeGenerated *) node;
    ...
    if (callContext == GEO_CONTEXT_RENDER) {
        ...
        if (asGenerated->parameter == bodyState->punchState >> 6) {
            if (sMarioAttackAnimCounter[gCurGraphNodeObjectNode->oUnk94] != gAreaUpdateCounter && (bodyState->punchState & PUNCH_STATE_TIMER_MASK) > 0) {
                bodyState->punchState -= 1;
                sMarioAttackAnimCounter[gCurGraphNodeObjectNode->oUnk94] = gAreaUpdateCounter;
            }
            scaleNode->scale =
...
Gfx *geo_switch_mario_hand_grab_pos(s32 callContext, struct GraphNode *node, Mat4 *mtx) {
    ...
    struct MarioState *marioState = &gMarioStates[gCurGraphNodeObjectNode->oUnk94];
